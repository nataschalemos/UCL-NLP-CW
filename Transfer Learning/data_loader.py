...
class GoEmotionsProcessor(object):
    ...
    def get_examples(self, mode, source):
        """
        Args:
            mode: train, dev, test
        """
        ...
        if source == 0:
            ...
            examples = self._create_examples(self._read_file(os.path.join(self.args.original_data_dir,
                                                               file_to_read)), mode, 0)
        elif source == 1:
            # Load group dataset (source=1)
            logger.info("LOOKING AT {}".format(os.path.join(self.args.group_data_dir, file_to_read)))
            examples = self._create_examples(self._read_file(os.path.join(self.args.group_data_dir,
                                                               file_to_read)), mode, 1)

        return examples


def load_and_cache_examples(args, tokenizer, mode, source):
    processor = GoEmotionsProcessor(args)
    ...
    if os.path.exists(cached_features_file):
        ...
        if mode == "train":
            examples = processor.get_examples("train", source)
        elif mode == "dev":
            examples = processor.get_examples("dev", source)
        elif mode == "test":
            examples = processor.get_examples("test", source)
        else:
